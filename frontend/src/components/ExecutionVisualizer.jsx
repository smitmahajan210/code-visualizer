  ...
  if (!code?.trim()) {
    return (
      <div className={`flex flex-col items-center justify-center flex-1 h-64 rounded-xl border-2 border-dashed text-center p-8
        ...
        <p className="text-sm mt-1 opacity-70">Write Python, JavaScript, TypeScript, Java, or C++ code in the editor, then click Run &amp; Trace</p>
      </div>
    );
  }
    ...
    <div className="flex flex-col gap-4 h-full">
      ...
      {traceResult && steps.length > 0 && (
        <div className="flex flex-col gap-4 flex-1">
          ...
          {/* Output */}
          <OutputPanel
            stdout={hasPerStepStdout ? (currentStep?.stdout || '') : (traceResult?.output || '')}
            darkMode={darkMode}
          />
        </div>
      )}
